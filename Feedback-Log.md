...
## [2026-06-16] PRD 完整需求拆解 & 进度追踪

**类型：** `项目管理`
**状态：** `已记录` — 详见 [`docs/PRD-TRACKER.md`](docs/PRD-TRACKER.md)

**内容摘要：**

对照 citation-checker-prd.pdf 逐条拆解了所有功能需求，标注了每个子需求的实现状态（✅/🔶/❌/⏸），覆盖：

- **§4.1 引用类型识别** — 论文/法条已完善，URL 类型缺失（M1）
- **§4.2 来源抓取** — 判例/法条/论文/书籍已有 fetcher，网页抓取和 PDF 解析缺失（M1/M3）
- **§4.3 比对校验** — Gemini LLM 方案已超越 PRD 预期，但缺少差异高亮
- **§5 技术方案** — 实际方案与 PRD 计划有合理差异（LLM 替代 embedding）
- **§7 输入/输出** — .docx 输入和 JSON 导出已完成，PDF 报告导出缺失（M4）
- **§8 验收标准** — 量化指标框架待建立
- **§9 迭代规划** — M2 已基本完成，M1/M3/M4 各有部分缺口

同时列出了 6 个可直接开工的任务和 4 个需要 PM 讨论的任务。
